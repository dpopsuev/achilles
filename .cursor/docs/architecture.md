...
| `Render` (Mermaid) | Circuit visualization via `achilles render` |
| `WalkObserver` | Live event trace during scan |

## What Achilles does NOT use (yet)

Adversarial Dialectic, Masks, Team Walk, Cycles, Ouroboros (metacalibration) — available via Origami but not needed for the current circuit.
